...
After a scan, open the **Ask** tab to pose natural-language questions about your findings (e.g. unknown BLE devices, strong Wi-Fi signals, risk score). Answers are generated by a **local** LiteRT/MediaPipe model ([cil-graph](https://github.com/contextgraph/cil-graph) android stack) — scan payloads are not uploaded. Download a recommended `.task` preset once over HTTPS, or import your own checkpoint via **Pick model**.
